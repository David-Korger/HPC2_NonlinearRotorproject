...
struct timeval tv_solve[50];
#define TIME_SAVE(j)   (gettimeofday(&tv_solve[j], (struct timezone*)0))
#define TIME_ELAPSED(j,k)	(1.E+6*(tv_solve[k].tv_sec - tv_solve[j].tv_sec)+(tv_solve[k].tv_usec - tv_solve[j].tv_usec))
