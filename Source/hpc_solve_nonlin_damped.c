...
struct timeval tv_solve_damped[50];
#define TIME_SAVE(j)   (gettimeofday(&tv_solve_damped[j], (struct timezone*)0))
#define TIME_ELAPSED(j,k)	(1.E+6*(tv_solve_damped[k].tv_sec - tv_solve_damped[j].tv_sec)+(tv_solve_damped[k].tv_usec - tv_solve_damped[j].tv_usec))

index hpc_solve_nonlin_damped(mesh **H, sed **A, index N, double *x,         
...
//   H[N]->fixed = mesh_getFixed(ncoord, bdry, nbdry, &H[N]->nfixed);
  ...
  printf("scal_tol = %10g\n",scal_tol);
  /* loop in damped Newton iteration */
  while ( iter < maxiter ){
    ...
    /* apply damping to correction of x: x_k+1 = x_k - damping * s */
    for (k = 0; k < n; k++) x[k] -= damping * w[k];
    iter++;
  }
