...
void mesh_vol_elem(double p1[2], double p2[2], double p3[2], index typ, 
                   double (*fvol)(double *, index), 
                   void (*fdiv)(double *, index, double *), double m[3])  
{
  int i;
  double mid[2], d[3][2], fac, fH[2] ;
  ...
  fac = fvol(mid,typ) * (d[1][0]*d[2][1]-d[2][0]*d[1][1]) / 6.0;
  fdiv(mid,typ,fH);     
  for ( i = 0 ; i < 3 ; i++ ){
    m[i] = fac; 
    m[i] += 0.5 * (fH[1]*d[i][0]-fH[0]*d[i][1]); 
  }
}
...
void mesh_vol(double p1[2], double p2[2], double p3[2], index  typ, 
              double (*fvol)(double *, index), 
              void (*fdiv)(double *, index, double *), double b[6]) 
{
  int j;
  ...
  mesh_vol_elem(p1, m[0], m[2], typ, fvol, fdiv, s);
  b[0]  = s[0]; b[3]  = s[1]; b[5]  = s[2];
  mesh_vol_elem(p2, m[1], m[0], typ, fvol, fdiv, s);
  b[1]  = s[0]; b[4]  = s[1]; b[3] += s[2];
  mesh_vol_elem(p3, m[2], m[1], typ, fvol, fdiv, s);
  b[2]  = s[0]; b[5] += s[1]; b[4] += s[2];
  mesh_vol_elem(m[1], m[2], m[0], typ, fvol, fdiv, s);
...
{  
  index j, k, nC, nT, nB, nE, *Elem, *Bdry, ind[6] ;
  double *Coord, bx[6] ;
  ...
  for ( k = 0 ; k < nT; k++)
  {
    ...
    mesh_vol(Coord+2*ind[0],Coord+2*ind[1],Coord+2*ind[2],Elem[7*k+6],fV,fDiv,bx);
